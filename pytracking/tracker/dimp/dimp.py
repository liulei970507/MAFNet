...
class DiMP(BaseTracker):
    ...
    def track(self, image, info: dict = None) -> dict:
        ...
        backbone_feat, sample_coords, im_patches = self.extract_backbone_features(im, self.get_centered_sample_pos(),
                                                                      ...
                                                                      self.img_sample_sz)
        with torch.no_grad():
            mp = self.net.mp_layer(backbone_feat['layer3']).sigmoid().item()
            backbone_feat['layer2'] = mp*self.net.rgb_branch_layer2(backbone_feat['layer2']) + (1-mp)*self.net.nir_branch_layer2(backbone_feat['layer2'])
            backbone_feat['layer3'] = mp*self.net.rgb_branch_layer3(backbone_feat['layer3']) + (1-mp)*self.net.nir_branch_layer3(backbone_feat['layer3'])
        
        # print(self.frame_num, mp)
    ...
    def generate_init_samples(self, im: torch.Tensor) -> TensorList:
        ...
        with torch.no_grad():
            ...
            mp = self.net.mp_layer(init_backbone_feat['layer3']).sigmoid().mean().item()
            init_backbone_feat['layer2'] = mp*self.net.rgb_branch_layer2(init_backbone_feat['layer2']) + (1-mp)*self.net.nir_branch_layer2(init_backbone_feat['layer2'])
            init_backbone_feat['layer3'] = mp*self.net.rgb_branch_layer3(init_backbone_feat['layer3']) + (1-mp)*self.net.nir_branch_layer3(init_backbone_feat['layer3'])

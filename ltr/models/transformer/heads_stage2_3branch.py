...
class Head(nn.Module):
    """
    """
    def __init__(self, filter_predictor, feature_extractor, classifier, bb_regressor,transformer_branch_RGB,
                 transformer_branch_NIR, transformer_branch_FUSION, separate_filters_for_cls_and_bbreg=False):
        super().__init__()
        self.transformer_branch_RGB = transformer_branch_RGB
        self.transformer_branch_NIR = transformer_branch_NIR
        self.transformer_branch_FUSION = transformer_branch_FUSION
    ...
    def forward(self, train_feat, test_feat, train_bb, *args, **kwargs):
        ...
        test_feat = test_feat.permute(1, 2, 0, 3, 4).flatten(2).permute(2, 0, 1) # Nf_te*H*W, Ns, C       
        train_feat = train_feat.permute(1, 2, 0, 3, 4).flatten(2).permute(2, 0, 1) # Nf_tr*H*W, Ns, C      
        #pdb.set_trace()
        test_pos = test_pos.permute(1, 2, 0, 3, 4).flatten(2).permute(2, 0, 1)
        train_pos = train_pos.permute(1, 2, 0, 3, 4).flatten(2).permute(2, 0, 1) 
        train_feat_rgb = self.transformer_branch_RGB(train_feat, train_pos)
        train_feat_nir = self.transformer_branch_NIR(train_feat, train_pos)
        train_feat = self.transformer_branch_FUSION(train_feat_rgb+train_feat_nir, train_feat_rgb, train_feat_nir, train_pos)
        
        test_feat_rgb = self.transformer_branch_RGB(test_feat, test_pos)
        test_feat_nir = self.transformer_branch_NIR(test_feat, test_pos)
        test_feat = self.transformer_branch_FUSION(test_feat_rgb+test_feat_nir, test_feat_rgb, test_feat_nir, test_pos)
        #pdb.set_trace()
        test_feat=test_feat.permute(1,2,0).view(b,c,f,w,h).permute(2,0,1,3,4)
        train_feat=train_feat.permute(1,2,0).view(b,c,2*f,w,h).permute(2,0,1,3,4)

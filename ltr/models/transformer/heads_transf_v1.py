...
class Head(nn.Module):
    """
    """
    def __init__(self, filter_predictor, feature_extractor, classifier, bb_regressor,transformer_fusion,
    ...
    def forward(self, train_feat_v,train_feat_t, test_feat_v,test_feat_t, train_bb, *args, **kwargs):
        ...
        test_pos = self.get_positional_encoding(test_feat_v) # Nf_te, Ns, C, H, W
        train_pos = self.get_positional_encoding(train_feat_v) # Nf_tr, Ns, C, H, W        
        test_feat_v = test_feat_v.permute(1, 2, 0, 3, 4).flatten(2).permute(2, 0, 1) # Nf_te*H*W, Ns, C
        test_feat_t = test_feat_t.permute(1, 2, 0, 3, 4).flatten(2).permute(2, 0, 1) # Nf_te*H*W, Ns, C        
        train_feat_v = train_feat_v.permute(1, 2, 0, 3, 4).flatten(2).permute(2, 0, 1) # Nf_tr*H*W, Ns, C
        train_feat_t = train_feat_t.permute(1, 2, 0, 3, 4).flatten(2).permute(2, 0, 1) # Nf_tr*H*W, Ns, C        
        #pdb.set_trace()
        test_pos = test_pos.permute(1, 2, 0, 3, 4).flatten(2).permute(2, 0, 1)
        train_pos = train_pos.permute(1, 2, 0, 3, 4).flatten(2).permute(2, 0, 1) 
        train_feat=self.transformer_fusion(train_feat_v,train_feat_t,train_pos)
        test_feat=self.transformer_fusion(test_feat_v,test_feat_t,test_pos)
        #pdb.set_trace()

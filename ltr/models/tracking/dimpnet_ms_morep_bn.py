...
class Modality_Branch(nn.Module):
    def __init__(self, input_channel):
        super().__init__()
        self.modality_specific = nn.Sequential(nn.Conv2d(input_channel,input_channel,1), nn.BatchNorm2d(input_channel),nn.ReLU(inplace=True))
        self.branches_top = nn.Sequential(nn.Conv2d(input_channel,input_channel//2,1), nn.BatchNorm2d(input_channel//2),nn.ReLU(inplace=True),nn.Conv2d(input_channel//2, input_channel//2, kernel_size=3, stride=1, padding=1), nn.BatchNorm2d(input_channel//2), nn.ReLU(inplace=True))
        self.branches_bottom = nn.Sequential(nn.Conv2d(input_channel,input_channel//2,1), nn.BatchNorm2d(input_channel//2),nn.ReLU(inplace=True),nn.Conv2d(input_channel//2, input_channel//2, kernel_size=(1,3), stride=(1,1), padding=(0,1)), nn.BatchNorm2d(input_channel//2), nn.ReLU(inplace=True),nn.Conv2d(input_channel//2, input_channel//2, kernel_size=(3,1), stride=(1,1), padding=(1,0)), nn.BatchNorm2d(input_channel//2), nn.ReLU(inplace=True))
...
class DiMPnet(nn.Module):
    ...
    def __init__(self, feature_extractor, classifier, bb_regressor, classification_layer, bb_regressor_layer, mp_layer, rgb_branch_layer2, rgb_branch_layer3, nir_branch_layer2, nir_branch_layer3):
        super().__init__()
        ...
        self.rgb_branch_layer2 = rgb_branch_layer2
        self.rgb_branch_layer3 = rgb_branch_layer3
        self.nir_branch_layer2 = nir_branch_layer2
        self.nir_branch_layer3 = nir_branch_layer3
    ...
    def forward(self, train_imgs, test_imgs, train_bb, test_proposals, *args, **kwargs):
        ...
        mp_train = self.mp_layer(train_feat['layer3'])
        mp_test = self.mp_layer(test_feat['layer3'])
        # import pdb
        # pdb.set_trace()
        # Classification features
        train_feat['layer2'] = mp_train.sigmoid().unsqueeze(-1).unsqueeze(-1)*self.rgb_branch_layer2(train_feat['layer2']) + (1-mp_train.sigmoid().unsqueeze(-1).unsqueeze(-1))*self.nir_branch_layer2(train_feat['layer2'])
        train_feat['layer3'] = mp_train.sigmoid().unsqueeze(-1).unsqueeze(-1)*self.rgb_branch_layer3(train_feat['layer3']) + (1-mp_train.sigmoid().unsqueeze(-1).unsqueeze(-1))*self.nir_branch_layer3(train_feat['layer3'])
        test_feat['layer2'] = mp_test.sigmoid().unsqueeze(-1).unsqueeze(-1)*self.rgb_branch_layer2(test_feat['layer2']) + (1-mp_test.sigmoid().unsqueeze(-1).unsqueeze(-1))*self.nir_branch_layer2(test_feat['layer2'])
        test_feat['layer3'] = mp_test.sigmoid().unsqueeze(-1).unsqueeze(-1)*self.rgb_branch_layer3(test_feat['layer3']) + (1-mp_test.sigmoid().unsqueeze(-1).unsqueeze(-1))*self.nir_branch_layer3(test_feat['layer3'])
        ...
        return target_scores, iou_pred, mp_train, mp_test
...
def dimpnet50(filter_size=1, optim_iter=5, optim_init_step=1.0, optim_init_reg=0.01,
    ...
    rgb_branch_layer2 = Modality_Branch(512)
    rgb_branch_layer3 = Modality_Branch(1024)
    nir_branch_layer2 = Modality_Branch(512)
    nir_branch_layer3 = Modality_Branch(1024)
    # DiMP network
    net = DiMPnet(feature_extractor=backbone_net, classifier=classifier, bb_regressor=bb_regressor,
                  classification_layer=classification_layer, bb_regressor_layer=['layer2', 'layer3'], mp_layer=mp_layer, rgb_branch_layer2=rgb_branch_layer2, rgb_branch_layer3=rgb_branch_layer3,
                  nir_branch_layer2=nir_branch_layer2, nir_branch_layer3=nir_branch_layer3)
    
    usepretrain = True
    if usepretrain:
        pretrainmodel_path='/data/liulei/RGBNIR/DIMP_AL/ltr/checkpoints/ltr/dimp/dimp50_baseline_rgbnir_all/DiMPnet_ep0050.pth.tar'
